...
Так как у нас в политиках безопасности фигурируют всего две сети источников (10.40.0.0 и 10.30.0.0), можно совместить все четыре политики в два списка доступа.

#### Шаг 2. Разработка и применение расширенных списков доступа, которые будут соответствовать требованиям политики безопасности.

Первый список доступа
```
ip access-list extended NetworkSales
 ...
 deny tcp 10.40.0.0 0.0.0.255 host 10.50.0.5 eq www
 deny tcp 10.40.0.0 0.0.0.255 host 10.50.0.5 eq 443
 permit ip any any
```
Для того, чтобы проверить запрет HTTP и HTTPS, добавляем в список строку с адресом нашего сервера. где мы поднял сервис HTTP.

Его прикрепляем на подинтерфейс g0/0/1.40 роутера R1.
```
R1(config)#int g0/0/1.40
R1(config-subif)#ip access-group NetworkSales in
```

Второй список доступа
```
R1(config)#ip access-list extended OperationsICMP
R1(config-ext-nacl)#deny icmp 10.30.0.0 0.0.0.255 10.40.0.0 0.0.0.255 echo
R1(config-ext-nacl)#permit ip any any
```
Его прикрепляем на подинтерфейс g0/0/1.30 роутера R1.
```
R1(config)#int g0/0/1.30
R1(config-subif)#ip access-group OperationsICMP in
```
