...
Какие группы многоадресной рассылки назначены интерфейсу G0/0?
 FF02::1            - мультигруппа всех  адресов ipv6 (аналог broadcast в ipv4)
 FF02::1:FF00:1     - мультигруппа запрашиваемых адресов (группа соответствует адресу сетевой карты)
...
b.	Активируйте IPv6-маршрутизацию на R1 с помощью команды IPv6 unicast-routing, чтобы убедиться, что новая многоадресная группа назначена интерфейсу G0/0/0. 
Обратите внимание, что в списке групп для интерфейса G0/0 отображается группа многоадресной рассылки всех маршрутизаторов (FF02::2).
```
R1#show ipv6 interface g0/0/0
Joined group address(es):
    FF02::1
    FF02::2    Появилась мультирассылка маршрутизаторов
    FF02::1:FF00:1
```
...
Сначала проверяем автоматические настройки сетевой карты (Ставим получение настроек автоматически):
...
FastEthernet0 Connection:(default port)
   ...
   DNS Servers.....................: ::
                                     0.0.0.0
```
Потом устанавливаем статические настройки сетевой карты.
...
2.	
Адрес FE80::1 может быть назначен обоим интерфейсам Ethernet на R1, поскольку каждый интерфейс маршрутизатора относится к отдельной сети. Пакеты с локальным адресом канала никогда не выходят за пределы локальной сети, а значит, для обоих интерфейсов можно указывать один и тот же локальный адрес канала.

3.	Какой идентификатор подсети в индивидуальном IPv6-адресе 2001:db8:acad::aaaa:1234/64?

Структура адреса: 

Первые 48 бит — префикс.       2001:db8:acad:0000

Следующие 16 бит — идентификатор подсети.   0000 

Последние 64 бита — идентификатор интерфейса.    0000:0000:aaaa:1234
